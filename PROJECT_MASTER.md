...
#### Phase 2.5: Multi-Model Dynamic Workflow (Priority 2) ✅ 核心完成
...
**🎉 测试结果** (2025-10-26):
- ✅ **T2I**: FLUX Dev 成功生成图片（https://v3b.fal.media/...）
- ✅ **I2V**: Kling V1 成功生成视频（3个视频，各5秒）
- ✅ **TTS**: VibeVoice 成功生成配音（4.4秒 MP3）
- ⚠️ **Merge**: MVP 模式（返回第一个视频，待集成 Shotstack）
- ⚠️ **视频时长**: 目前固定5秒（Kling不支持duration参数控制）
...
- [x] **Task 4.8**: 扩展模型配置接口 ✅
  - [x] 添加 `capabilities` 字段到 `ModelOption`
  - [x] 定义音频生成能力元数据
  - [x] 更新真实可用模型配置：
    - FLUX Dev/Schnell/LoRA（T2I）
    - Kling V1/V1.6, MiniMax, LTX（I2V）
    - VibeVoice 1.5B/7B（TTS）
  - [x] 文件: `src/config/models.ts`

- [x] **Task 4.9**: WorkflowBuilder 实现 ✅
  - [x] 智能节点生成（根据模型能力决定 T2I→I2V 或直接 T2V）
  - [x] 音频策略决策（4种场景）
  - [x] 依赖关系计算
  - [x] 文件: `src/services/workflow-builder.ts`

- [x] **Task 4.10**: 真实 API 集成测试 ✅
  - [x] 修复 Fal.ai 模型 ID 格式（404 → 200）
  - [x] 修复响应数据结构解析（result.data → result）
  - [x] 修复 Kling 参数格式（移除不支持的参数）
  - [x] 切换 TTS 到 VibeVoice（400 → 200）
  - [x] 端到端测试成功（8秒视频，3场景）
...
- [x] **Task 4.11**: 拓扑排序算法 ✅
  - [x] 根据节点依赖关系计算执行顺序
  - [x] 检测循环依赖
  - [x] DFS 实现（src/app/api/runs/[runId]/execute/route.ts）

- [x] **Task 4.12**: 动态输入解析 ✅
  - [x] 支持节点引用语法：`{{nodeId.outputKey}}`
  - [x] 运行时解析输入参数
  - [x] 支持 fallback 语法：`{{i2v-0.videoUrl || t2v-0.videoUrl}}`

- [ ] **Task 4.13**: 音频混合实现 ⏸️ 待 Shotstack 集成
  - [ ] 实现 AudioMixer 节点
  - [ ] 支持多轨混音（视频音效 + TTS + 背景音乐）
  - [ ] 音量平衡控制（可配置比例）
  - [ ] 当前状态：MVP 返回第一个视频

- [x] **Task 4.14**: Orchestrator 重构 ✅
  - [x] 从固定流程改为动态解析
  - [x] 支持任意工作流节点组合
  - [x] 集成 Fal.ai T2I/I2V/TTS 适配器
  - [x] 文件: `src/app/api/runs/[runId]/execute/route.ts`

- [x] **Task 4.15**: 集成测试 ✅
  - [x] 测试 Kling V1 组合（T2I → I2V → TTS → Merge）
  - [x] 验证拓扑排序正确性
  - [x] 验证参数引用解析
...
**📋 Phase 2.5 已知问题与优化计划** (2025-10-26)

**已知问题**:
1. ⚠️ **视频时长固定5秒** - Kling V1 不支持 duration 参数，需要测试其他模型
2. ⚠️ **视频未拼接** - Merge 节点返回第一个视频，需要集成 Shotstack API
3. ⚠️ **文件临时存储** - Fal.ai CDN 有效期24-48小时，需要持久化到 Supabase Storage
4. ⚠️ **节点无预览** - 执行完成后无法在画布上预览生成结果
5. ⚠️ **角色一致性** - 多个场景中的角色可能不一致，需要 IP-Adapter

**下一步优化计划** (按优先级):

**Phase 2.6: 用户体验优化** (优先级 ⭐⭐⭐)
- [ ] Task 4.19: 节点结果预览
  - [ ] 监听 SSE `node_complete` 事件更新节点数据
  - [ ] T2I 节点显示图片预览（可点击放大）
  - [ ] I2V 节点显示视频播放器
  - [ ] TTS 节点显示音频播放器
  - [ ] 预计时间：1天

**Phase 2.7: 视频拼接集成** (优先级 ⭐⭐⭐)
- [ ] Task 4.20: Shotstack API 集成
  - [ ] 申请 Shotstack API Key
  - [ ] 实现视频拼接逻辑（多视频 + 音频混合）
  - [ ] 支持转场效果（可选）
  - [ ] 预计时间：1天

**Phase 2.8: 文件持久化** (优先级 ⭐⭐)
- [ ] Task 4.21: Supabase Storage 集成
  - [ ] 下载 Fal.ai 生成的文件
  - [ ] 上传到 Supabase Storage（永久存储）
  - [ ] 更新数据库 artifact 记录
  - [ ] 预计时间：0.5天
...
- [ ] Task 4.23: IP-Adapter 集成
  - [ ] 使用 FLUX General 模型（支持 IP-Adapter）
  - [ ] 传递参考图 URL 到所有 T2I 节点
  - [ ] 调整 `image_prompt_strength` 参数
  - [ ] 预计时间：0.5天

**Phase 2.10: 视频时长控制** (优先级 ⭐)
- [ ] Task 4.24: 测试支持 duration 的模型
  - [ ] 测试 MiniMax Video（是否支持 duration）
  - [ ] 测试 LTX Video
  - [ ] 如果不支持，考虑后期剪辑方案
  - [ ] 预计时间：0.5天
